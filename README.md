# 👋 Hi, I'm Utkarsh Katiyar

### 📊 Data Analysis | 🤖 Aspiring ML/DL Engineer

Engineering student passionate about **Data Science, Machine Learning and AI**.

📊 Strong in **Data Analysis, Python, SQL, Visualization & BI**  
🤖 Currently learning **Machine Learning & Deep Learning**  
☁️ Foundational knowledge of **AWS & Azure**  
🧩 Practicing **DSA with C++**  
🎯 Aspiring to build **data-driven and intelligent applications**

📫 **Email:** `utkarshkatiyarmaverick@gmail.com`

---

# 🛠️ Tech Stack

## 👨‍💻 Programming Languages

<p align="left">
<img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/python/python-original.svg" width="45"/>
<img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/cplusplus/cplusplus-original.svg" width="45"/>
</p>

**Python** • **C++**

---

# 📊 Data Science & Data Analysis

### 🔬 Libraries

<p align="left">
<img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/numpy/numpy-original.svg" width="45"/>
<img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/pandas/pandas-original.svg" width="45"/>
<img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/matplotlib/matplotlib-original.svg" width="45"/>
<img src="https://seaborn.pydata.org/_images/logo-mark-lightbg.svg" width="45"/>
</p>

`Data Cleaning` • `EDA` • `Statistical Analysis` • `Correlation` • `Outlier Detection` • `Data Visualization`

---

# 🤖 Machine Learning

### 🔬 Scikit-Learn

`Regression` • `Classification` • `Clustering` • `Feature Engineering` • `Feature Selection` • `Model Evaluation` • `Cross Validation` • `Hyperparameter Tuning`

---

# 🧠 Deep Learning

### 🚧 Currently Learning

`Neural Networks` • `CNN` • `Image Classification` • `Computer Vision` • `Model Training`

---

# 🗄️ Databases

<p align="left">
<img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/mysql/mysql-original.svg" width="45"/>
<img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/postgresql/postgresql-original.svg" width="45"/>
<img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/oracle/oracle-original.svg" width="45"/>
<img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/mongodb/mongodb-original.svg" width="45"/>
</p>

**MySQL** • **PostgreSQL** • **Oracle** • **MongoDB**

`SELECT` • `WHERE` • `GROUP BY` • `HAVING` • `JOINS` • `SUBQUERIES` • `CTEs` • `WINDOW FUNCTIONS`

---

# 📈 Business Intelligence & Analytics

<p align="left">
<img src="https://img.icons8.com/color/96/power-bi.png" width="45"/>
<img src="https://img.icons8.com/color/96/microsoft-excel-2019.png" width="45"/>
</p>

**Power BI** • **Excel**

`Dashboards` • `KPIs` • `Data Visualization` • `Business Insights` • `Data Analysis`

---

# 🌐 Development & Deployment

<p align="left">
<img src="https://streamlit.io/images/brand/streamlit-mark-color.png" width="45"/>
<img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/html5/html5-original.svg" width="45"/>
<img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/css3/css3-original.svg" width="45"/>
</p>

**Streamlit** • **HTML5** • **CSS3**

---

# ☁️ Cloud Technologies

<p align="left">
<img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/amazonwebservices/amazonwebservices-original-wordmark.svg" width="55"/>
<img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/azure/azure-original.svg" width="45"/>
</p>

**AWS** — Cloud Practitioner-level knowledge  
**Microsoft Azure** — Fundamentals-level knowledge

---

# 🧰 Tools

<p align="left">
<img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/vscode/vscode-original.svg" width="45"/>
<img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/jupyter/jupyter-original.svg" width="45"/>
<img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/git/git-original.svg" width="45"/>
<img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/github/github-original.svg" width="45"/>
</p>

**VS Code** • **Jupyter Notebook** • **Git** • **GitHub**

---

# 🧩 Complete Technology Stack

<p align="center">
<img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/python/python-original.svg" width="45"/>
<img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/cplusplus/cplusplus-original.svg" width="45"/>
<img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/numpy/numpy-original.svg" width="45"/>
<img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/pandas/pandas-original.svg" width="45"/>
<img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/matplotlib/matplotlib-original.svg" width="45"/>
<img src="https://seaborn.pydata.org/_images/logo-mark-lightbg.svg" width="45"/>
<img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/mysql/mysql-original.svg" width="45"/>
<img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/postgresql/postgresql-original.svg" width="45"/>
<img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/oracle/oracle-original.svg" width="45"/>
<img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/mongodb/mongodb-original.svg" width="45"/>
<img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/tensorflow/tensorflow-original.svg" width="45"/>
<img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/html5/html5-original.svg" width="45"/>
<img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/css3/css3-original.svg" width="45"/>
<img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/amazonwebservices/amazonwebservices-original-wordmark.svg" width="55"/>
<img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/azure/azure-original.svg" width="45"/>
<img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/vscode/vscode-original.svg" width="45"/>
<img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/jupyter/jupyter-original.svg" width="45"/>
<img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/git/git-original.svg" width="45"/>
<img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/github/github-original.svg" width="45"/>
</p>

---

# 📚 Currently Learning

`Machine Learning` → `Deep Learning` → `Computer Vision` → `ML Deployment`

---

# 🚀 Projects

### 🩺 Medical Recommendation System
**Python • Machine Learning • Pandas • NumPy**

End-to-end ML recommendation system that processes **user-reported symptoms** and generates treatment and precaution recommendations.

### 💬 WhatsApp Text Analyzer
**Python • NLP • Pandas • Matplotlib • Streamlit**

Interactive application for analyzing WhatsApp chat logs with **message frequency, user activity, word clouds, sentiment analysis and communication insights**.

---

# 🧩 Problem Solving

Practicing **Data Structures & Algorithms using C++**.

---

# 📊 GitHub Analytics

<p align="center">
<img src="https://github-readme-stats.vercel.app/api?username=katiyarutkarsh2006-pixel&show_icons=true&locale=en" height="170"/>
<img src="https://github-readme-stats.vercel.app/api/top-langs?username=katiyarutkarsh2006-pixel&show_icons=true&locale=en&layout=compact" height="170"/>
</p>

---

# 🐍 Contribution Graph

<p align="center">
<img src="https://raw.githubusercontent.com/katiyarutkarsh2006-pixel/katiyarutkarsh2006-pixel/output/github-contribution-grid-snake.svg"/>
</p>

---

# 🎯 2026 Goals

**Master ML & DL** • **Build End-to-End AI Projects** • **Improve DSA** • **Learn ML Deployment & Cloud** • **Contribute to Open Source**

---

# 🤝 Open to Opportunities

**Data Analyst • Data Science • ML/AI • Open Source • Hackathons**

---

# 🔗 Connect With Me

📧 `utkarshkatiyarmaverick@gmail.com`

[LinkedIn](https://www.linkedin.com/in/utkarsh-katiyar) •
[LeetCode](https://leetcode.com/uttu_02) •
[Instagram](https://instagram.com/utkarsh_katiyar._)

### 🚀 Learn → Build → Debug → Improve
